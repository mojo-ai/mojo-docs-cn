...
## “值的生命周期”：关于值的诞生、存在和消亡[](#value-lifecycle-birth-life-and-death-of-a-value)
...
C++ 代码有许多与值生命周期相交的 Magic 运算符，例如“placement new”、“placement delete”和“operator=”，它们会在现有值上重新分配。当您使用 Mojo 的所有语言功能并在安全结构之上进行组合时，Mojo 是一种安全语言，但任何堆栈都是 C 风格指针和猖獗的不安全性的世界。Mojo 是一种实用语言，由于我们对与 C/C++ 互操作以及直接在 Mojo 本身中实现安全结构（如 String）感兴趣，因此我们需要一种方法来表达不安全的事物。
